...
test("only exact same-origin static URLs use cache; version and Supabase requests bypass it", async () => {
  const h = harness(); await h.lifecycle("install"); h.setOnline(false);
  for (const url of ["./style.css?v=195", "./script.js?v=107", "./roulette-service.js?v=9", "./roulette-offline-queue.js?v=3", "./assets/menu-background.webp", "./assets/sidemenu-background.webp", "./assets/gold-feld.webp?v=1"]) {
...
test("updated shell uses the exact new script URL offline even if the old version remains cached", async () => {
  ...
  assert.ok(bootUrls.includes("./script.js?v=107"));
  assert.equal((await h.request("./script.js?v=107")).url, new URL("./script.js?v=107", scope).href);
  assert.equal(await h.request("./script.js?v=108"), null);
});
