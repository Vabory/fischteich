...
test("kick wrapper submits one target intent and reloads server truth", async () => {
  const { service, rpcCalls } = createHarness();
  const targetPlayerId = "00000000-0000-4000-8000-000000000099";
  const snapshot = await service.kickPlayer(SESSION_ID, targetPlayerId);
  assert.equal(snapshot.session.id, SESSION_ID);
  assert.deepEqual(
    JSON.parse(JSON.stringify(rpcCalls.filter(({ name }) => name === "kick_trottl_classic_player"))),
    [{
      name: "kick_trottl_classic_player",
...
test("Klassik UI provides two rooms, lobby controls and the responsive game table", () => {
  assert.match(html, /trottl-classic-service\.js\?v=17[\s\S]*trottl-classic-preview\.js\?v=3[\s\S]*trottl-classic-ui\.js\?v=45[\s\S]*script\.js\?v=101/);
  ...
  assert.match(html, /class="visually-hidden" id="trottl-classic-rooms-title">Raum wählen<\/h1>[\s\S]*text-raum-wählen\.webp\?v=1[\s\S]*class="trottl-classic-room-context">3er Trottl<br>CLASSIC<\/p>/);
  ...
  assert.match(css, /\.trottl-classic-room-list[\s\S]*margin-top:\s*calc\(clamp\(54px, 10dvh, 88px\) \+ 26px - 0\.936rem\)[\s\S]*gap:\s*24px/);
  ...
  assert.match(ui, /trottlBadge\.textContent = "TROTTL"/);
...
test("navigation, reconnect and lifecycle cleanup are wired without touching the die", () => {
  ...
  assert.match(script, /async function restoreTrottlAfterAuth\(\)[\s\S]*ensureTrottlSpecial\(\)\.restoreMembership\(\)[\s\S]*await trottlClassic\.restoreMembership\(\)/);
  assert.match(script, /initializeAppAuth\(\)\.then\(restoreTrottlAfterAuth\)/);
