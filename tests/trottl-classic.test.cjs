...
test("Klassik UI provides two rooms, lobby controls and the responsive game table", () => {
  assert.match(html, /trottl-classic-service\.js\?v=17[\s\S]*trottl-classic-preview\.js\?v=3[\s\S]*trottl-classic-ui\.js\?v=44[\s\S]*script\.js\?v=95/);
...
test("navigation, reconnect and lifecycle cleanup are wired without touching the die", () => {
  ...
  assert.match(script, /initializeAppAuth\(\)\.then\(async \(\) =>[\s\S]*ensureTrottlSpecial\(\)\.restoreMembership\(\)[\s\S]*await trottlClassic\.restoreMembership\(\)/);
