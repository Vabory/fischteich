...
const source=read("trottl-special-color-chaos.js"),migration=read("supabase/migrations/20260917010000_add_trottl_special_color_chaos.sql");
...
test("spectator receives host challenge and progress but every fish remains inert",()=>{const h=harness({role:"spectator",progress:2,index:4});assert.match(h.root.querySelector(".trottl-special-minigame-copy").textContent,/Host-Sicht/);assert.equal(h.root.querySelector(".trottl-special-color-chaos-progress").textContent,"Fortschritt: 2 / 5");h.tap(h.view.target_color);assert.equal(h.calls.length,0);assert.ok(h.root.querySelectorAll(".trottl-special-color-chaos-choice").every(button=>button.disabled));});
...
test("production pool, debug selector and routing contain exactly productive IDs 01 through 04",()=>{const h=harness(),active=Array.from(h.win.TrottlSpecialMinigames.registry.filter(entry=>entry.active),entry=>entry.id);assert.deepEqual(active,["special_minigame_01","special_minigame_02","special_minigame_03","special_minigame_04"]);assert.ok(h.win.TrottlSpecialMinigames.registry.slice(4).every(entry=>!entry.active&&!entry.implemented));
 for(const id of ["01","02","03","04"])assert.match(migration,new RegExp(`chosen='special_minigame_${id}'`));assert.doesNotMatch(migration,/chosen='special_minigame_0[5-9]'|chosen='special_minigame_10'/);assert.match(read("trottl-special-debug.js"),/registry\.filter\(r => r\.active && r\.implemented\)/);});
...
test("CSS and wiring provide a fixed 2x2 mobile grid, responsive contain fish, fast feedback and reduced motion",()=>{const css=read("trottl-special.css"),html=read("index.html"),ui=read("trottl-special-ui.js"),service=read("trottl-special-service.js");assert.match(css,/color-chaos-grid[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);assert.match(css,/color-chaos-choice img[^}]*width: clamp\([^}]*object-fit: contain/);assert.match(css,/color-chaos-choice\.is-wrong/);assert.match(css,/prefers-reduced-motion: reduce[^}]*color-chaos-challenge/);assert.match(source,/correctDelayMs: 120, wrongDelayMs: 300, fadeOutMs: 150, fadeInMs: 150/);assert.match(source,/addEventListener\("pointerdown"/);assert.match(html,/trottl-special-color-chaos\.js\?v=1/);assert.match(ui,/colorChaos\?\.update\(snapshot\)/);assert.match(service,/answerColorChaos: async/);});
test("dedicated SQL fixture covers deterministic challenge and exact four-game pool",()=>{const fixture=read("tests/fixtures/trottl-special-color-chaos.sql");assert.match(fixture,/Deterministic private challenge/);assert.match(fixture,/Four-way production pool/);assert.ok(fixture.trim().endsWith("rollback;"));});
