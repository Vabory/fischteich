...
test("production pool, debug selector and routing contain exactly productive IDs 01 through 07",()=>{const h=harness(),active=Array.from(h.win.TrottlSpecialMinigames.registry.filter(entry=>entry.active),entry=>entry.id);assert.deepEqual(active,["special_minigame_01","special_minigame_02","special_minigame_03","special_minigame_04","special_minigame_05","special_minigame_06","special_minigame_07"]);assert.ok(h.win.TrottlSpecialMinigames.registry.slice(7).every(entry=>!entry.active&&!entry.implemented));
