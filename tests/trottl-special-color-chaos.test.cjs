...
test("CSS and wiring provide stable three-row geometry, a 2x2 grid, fast opacity fades and reduced motion",()=>{const css=read("trottl-special.css"),html=read("index.html"),ui=read("trottl-special-ui.js"),service=read("trottl-special-service.js");assert.match(css,/color-chaos-game[^}]*height: clamp\(300px, 43dvh, 390px\)[^}]*grid-template-rows:/);assert.match(css,/color-chaos-grid[^}]*height: 100%[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)[^}]*grid-template-rows: repeat\(2, minmax\(0, 1fr\)\)/);assert.match(css,/color-chaos-choice img[^}]*width: clamp\([^}]*object-fit: contain/);assert.match(css,/color-chaos-game\.is-leaving[^}]*opacity: 0/);assert.match(css,/prefers-reduced-motion: reduce[^}]*color-chaos-task-zone/);assert.doesNotMatch(css.match(/color-chaos-game\.is-leaving[\s\S]*?\}/)?.[0]??"",/display|height|margin/);assert.match(source,/correctDelayMs: 120, wrongDelayMs: 300, fadeOutMs: 150, fadeInMs: 150/);assert.match(source,/addEventListener\("pointerdown"/);assert.match(html,/trottl-special-color-chaos\.js\?v=2/);assert.match(ui,/colorChaos\?\.update\(snapshot\)/);assert.match(service,/answerColorChaos: async/);});
