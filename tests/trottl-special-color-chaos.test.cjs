...
test("all four mapped WebP assets preload and decode before play",async()=>{const h=harness();await flush();assert.deepEqual(h.images.map(image=>image.src),[
 "./assets/mini-games/red-fish.webp","./assets/mini-games/blue-fish.webp","./assets/mini-games/green-fish.webp","./assets/mini-games/yellow-fish.webp"]);});
...
test("production pool and debug selector contain exactly productive IDs 01 through 09",()=>{const h=harness(),active=Array.from(h.win.TrottlSpecialMinigames.registry.filter(entry=>entry.active),entry=>entry.id);assert.deepEqual(active,["special_minigame_01","special_minigame_02","special_minigame_03","special_minigame_04","special_minigame_05","special_minigame_06","special_minigame_07","special_minigame_08","special_minigame_09"]);assert.ok(h.win.TrottlSpecialMinigames.registry.slice(9).every(entry=>!entry.active&&!entry.implemented));
 for(const id of ["01","02","03","04","05","06"])assert.match(id==="06"?read("supabase/migrations/20260920010000_add_trottl_special_stop_fish.sql"):memoryMigration,new RegExp(`chosen='special_minigame_${id}'`));assert.match(read("trottl-special-debug.js"),/registry\.filter\(r => r\.active && r\.implemented\)/);});
