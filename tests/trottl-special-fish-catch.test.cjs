...
test("cache keeps Fischfang v3 while shared Special assets advance for Reaktionstest",()=>{const html=read("index.html");assert.match(html,/trottl-special-fish-catch\.js\?v=3/);assert.match(html,/trottl-special\.css\?v=16/);for(const [file,v]of [["trottl-special-minigames.js",3],["trottl-special-number-hunt.js",2],["trottl-special-panic.js",1],["trottl-special-roulette.js",3]])assert.ok(html.includes(`${file}?v=${v}`));});
