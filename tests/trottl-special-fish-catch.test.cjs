...
test("the central asset pool preloads exactly normal fish 1 through 8",()=>{
 ...
 assert.deepEqual(assets,Array.from({length:8},(_,i)=>`./assets/mini-games/${i+1}-fish.webp`));assert.deepEqual(h.preloaded,assets);
...
test("cache advances Fischfang while retaining the other Special asset versions",()=>{const html=read("index.html");assert.match(html,/trottl-special-fish-catch\.js\?v=4/);assert.match(html,/trottl-special\.css\?v=29/);for(const [file,v]of [["trottl-special-minigames.js",7],["trottl-special-number-hunt.js",2],["trottl-special-panic.js",1],["trottl-special-roulette.js",3]])assert.ok(html.includes(`${file}?v=${v}`));});
