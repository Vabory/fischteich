...
test("fixed registry has ten stable IDs with Zahlenjagd and Fischfang playable",()=>{
 const h=harness();assert.equal(h.registry.length,10);assert.deepEqual(Array.from(h.registry.filter(r=>r.active),r=>r.id),["special_minigame_01","special_minigame_02"]);
 assert.ok(h.registry.slice(2).every(r=>r.title===null && !r.implemented));
});
