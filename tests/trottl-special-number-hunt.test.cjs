...
test("fixed registry has ten stable IDs with the first five minigames playable",()=>{
 const h=harness();assert.equal(h.registry.length,10);assert.deepEqual(Array.from(h.registry.filter(r=>r.active),r=>r.id),["special_minigame_01","special_minigame_02","special_minigame_03","special_minigame_04","special_minigame_05"]);
 assert.ok(h.registry.slice(5).every(r=>r.title===null && !r.implemented));
});
