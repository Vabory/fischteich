...
test("fixed registry has ten stable IDs with the first nine minigames playable",()=>{
 const h=harness();assert.equal(h.registry.length,10);assert.deepEqual(Array.from(h.registry.filter(r=>r.active),r=>r.id),["special_minigame_01","special_minigame_02","special_minigame_03","special_minigame_04","special_minigame_05","special_minigame_06","special_minigame_07","special_minigame_08","special_minigame_09"]);
 assert.ok(h.registry.slice(9).every(r=>r.title===null && !r.implemented));
});
