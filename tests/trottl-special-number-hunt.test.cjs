...
test("fixed registry has ten stable IDs with the first three minigames playable",()=>{
 const h=harness();assert.equal(h.registry.length,10);assert.deepEqual(Array.from(h.registry.filter(r=>r.active),r=>r.id),["special_minigame_01","special_minigame_02","special_minigame_03"]);
 assert.ok(h.registry.slice(3).every(r=>r.title===null && !r.implemented));
});
