...
(function installNumberHunt(global) {
  function create({ root, service, onSnapshot, onError }) {
    ...
    let snapshot = null, key = null, queue = [], sending = false, timer = null, suspended = true, retryAt = 0, stoppedAt = null, viewKey = null;
    function localRun() {
    ...
    function count(run) { return Math.max(Number(run?.progress ?? 0), ...queue, 0); }
    function tick() {
      ...
      for (const [number, b] of buttons) {
        b.disabled = !eligible || spectator || s.phase !== "active" || n === 9 || number <= n;
    ...
    function tap(number) {
      ...
      if (global.TrottlSpecialMinigames.sequence(m, now).phase !== "active" || number !== count(run) + 1 || run?.completed) return;
      queue.push(number); if (number === 9) stoppedAt = Math.floor(Math.max(0, now - Date.parse(m.start_at)));
      tick(); void drain();
    }
    async function drain() {
      if (sending || suspended || !queue.length || !snapshot) return;
      ...
      try {
        while (!suspended && key === ownKey && queue.length) {
          const number = queue[0], next = await service.tapNumberHunt(id, round, number, number === 9 ? stoppedAt : null);
          if (key !== ownKey) return;
          queue = queue.filter(n => n > number); retryAt = 0;
          onSnapshot(next);
        }
      } catch (error) {
        if (key === ownKey) { retryAt = (service.serverNow() ?? 0) + 500; onError?.(error); }
    ...
    function update(next) {
      ...
      if (key !== nextKey) { key = nextKey; queue = []; sending = false; stoppedAt = null; retryAt = 0; viewKey = null; }
      const run = localRun().run;
      queue = queue.filter(n => n > Number(run?.progress ?? 0));
