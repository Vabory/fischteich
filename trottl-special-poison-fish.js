...
(function installTrottlSpecialPoisonFish(global) {
  ...
  function fishType(slot, config = CONFIG) { return slot < config.normalFishCount ? "normal" : slot < config.normalFishCount + config.goldFishCount ? "gold" : "poison"; }
  function fishAsset(slot, spawnIndex = 0, config = CONFIG) { const type = fishType(slot, config); return type === "normal" ? NORMAL_ASSETS[(slot + spawnIndex) % NORMAL_ASSETS.length] : type === "gold" ? GOLD_ASSET : POISON_ASSET; }
  ...
  function initialSlots(config = CONFIG) { return Array.from({ length: config.normalFishCount + config.goldFishCount + config.poisonFishCount }, () => ({ spawnIndex: 0, spawnAt: 0, previousX: null, previousY: null })); }
  function hitTest(seed, slots, elapsed, x, y, config = CONFIG) {
    let winner = null;
  ...
  function replay(seed, events, config = CONFIG) {
    const slots = initialSlots(config), hits = []; let score = 0;
    for (const event of events) {
      const hit = hitTest(seed, slots, event.t, event.x, event.y, config);
      if (!hit) continue;
      const type = fishType(hit.slot, config), points = type === "normal" ? 1 : type === "gold" ? 3 : -3;
  ...
  function create({ root, service, onSnapshot, onError }) {
    ...
    field.setAttribute("aria-label", "Giftfisch-Spielfeld"); field.classList.add("is-preview"); countdown.classList.add("trottl-special-poison-fish-countdown");
    ...
    let cachedRun = null, cachedEventCount = -1, cachedSeed = null, cachedVersion = null;
    const current = () => snapshot?.poisonFishView ?? null;
    const runConfig = () => CONFIG_BY_VERSION[current()?.simulation_version] ?? CONFIG;
    ...
    const elapsedAt = now => { const start = Date.parse(snapshot?.session.gameState.minigame?.start_at); return now === null || !Number.isFinite(start) ? null : Math.max(0, now - start - 400); };
    const elapsedNow = () => elapsedAt(service.serverNow());
    function localRun() { const seed = current()?.movement_seed, config = runConfig(); if (seed !== cachedSeed || events.length !== cachedEventCount || cachedVersion !== config.simulationVersion) {
      cachedSeed = seed; cachedEventCount = events.length; cachedVersion = config.simulationVersion; cachedRun = replay(seed, events, config); } return cachedRun; }
    ...
    function pointer(event) {
      const receivedAt = service.serverNow(), eventLag = global.performance && Number.isFinite(event.timeStamp) ? global.performance.now() - event.timeStamp : 0;
      const tapAt = receivedAt === null ? null : receivedAt - (eventLag >= 0 && eventLag <= 1000 ? eventLag : 0);
      const view = current(), config = runConfig(), elapsed = elapsedAt(tapAt);
      if (event.isTrusted === false || event.pointerType === "mouse" && event.button !== 0 || suspended || !ownPlayer() || view?.completed || !view || doc.visibilityState === "hidden" || elapsed === null || elapsed >= config.durationMs || global.TrottlSpecialMinigames.sequence(snapshot.session.gameState.minigame, tapAt).label !== "" || events.length >= config.maxEvents) return;
      ...
      const hit = hitTest(view.movement_seed, localRun().slots, entry.t, entry.x, entry.y, config);
      events.push(entry); if (hit) { const type = fishType(hit.slot, config); feedback({ type, points: type === "normal" ? 1 : type === "gold" ? 3 : -3 }, x, y); }
      paint(); void drain();
    }
    field.addEventListener("pointerdown", pointer);
    async function drain() {
      if (syncing || suspended || !snapshot || !ownPlayer() || service.serverNow() < retryAt) return;
      const view = current(), config = runConfig(), elapsed = elapsedNow(), shouldFinal = elapsed !== null && elapsed >= config.durationMs, now = service.serverNow();
    ...
    async function finalize() {
      const elapsed = elapsedNow(), now = service.serverNow(), duration = runConfig().durationMs;
      if (finalizing || suspended || !snapshot || elapsed === null || elapsed < duration + 2000 || now === null || now < finalizeRetryAt || ownPlayer() && (!finalSent || syncing) && elapsed < duration + 3000) return;
    ...
    function paint() {
      ...
      if (!geometry) measure();
      const config = runConfig(), run = localRun(), movementTime = preview ? 0 : Math.min(config.durationMs, elapsed ?? 0);
      ...
      fishNodes.forEach((image, slot) => {
        if (slot >= run.slots.length) return;
        ...
        if (drawnSpawns[slot] !== slotState.spawnIndex) {
          ...
          image.src = fishAsset(slot, slotState.spawnIndex, config); image.style.zIndex = String(drawnMovements[slot].z + 1); drawnSpawns[slot] = slotState.spawnIndex;
        }
    ...
    function update(next) {
      ...
      if (key !== nextKey) { key = nextKey; events = [...(next.poisonFishView?.events ?? [])]; lastSentLength = events.length; finalSent = next.poisonFishView?.completed === true; retryAt = 0; finalizeRetryAt = 0; nextCheckpointAt = 0; cachedEventCount = -1;
        ...
        const config = runConfig(), count = config.normalFishCount + config.goldFishCount + config.poisonFishCount;
        fishNodes.forEach((image, slot) => { image.style.visibility = slot < count ? "" : "hidden"; });
