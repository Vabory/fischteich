...
(function installTrottlSpecialPoisonFish(global) {
  const CONFIG = Object.freeze({ simulationVersion: 1, normalFishCount: 8, goldFishCount: 2, poisonFishCount: 3,
    durationMs: 10000, tickMs: 33, hitRadiusX: 0.08, hitRadiusY: 0.065, maxEvents: 500 });
  ...
  function fishAsset(slot) { const type = fishType(slot); return type === "normal" ? NORMAL_ASSETS[slot % NORMAL_ASSETS.length] : type === "gold" ? GOLD_ASSET : POISON_ASSET; }
  ...
  function stateAt(seed, slot, state, elapsed) {
    ...
    const delta = Math.max(0, elapsed - state.spawnAt), x = reflect(movement.x, movement.vx, delta, 0.1, 0.9), y = reflect(movement.y, movement.vy, delta, 0.09, 0.91);
  ...
  function create({ root, service, onSnapshot, onError }) {
    ...
    let snapshot = null, key = null, interval = null, suspended = true, syncing = false, finalizing = false, events = [], lastSentLength = 0, finalSent = false, retryAt = 0, finalizeRetryAt = 0;
    ...
    function pointer(event) {
      ...
      const bounds = field.getBoundingClientRect(), x = (event.clientX - bounds.left) / bounds.width, y = (event.clientY - bounds.top) / bounds.height;
    ...
    async function drain() {
      ...
      const view = current(), elapsed = elapsedNow(), shouldFinal = elapsed !== null && elapsed >= CONFIG.durationMs;
      ...
      try {
        ...
        lastSentLength = submitted.length; if (isFinal) finalSent = true;
        onSnapshot(next);
    ...
    function tick() {
      ...
      const now = service.serverNow(), view = current(), m = snapshot.session.gameState.minigame, frame = shell.frame(m, now), preview = frame.phase === "countdown" || frame.label === "START!", elapsed = elapsedNow();
      const visible = Boolean(view) && (preview || frame.phase === "active"); shell.panel.classList.toggle("is-gameplay", visible); shell.content.hidden = !visible; game.hidden = !visible;
      shell.copy.hidden = frame.phase !== "title"; if (!view) return;
      const run = localRun(), movementTime = preview ? 0 : Math.min(CONFIG.durationMs, elapsed ?? 0);
      scoreLabel.textContent = `Punkte: ${run.score}`;
      const seconds = Math.ceil(Math.max(0, CONFIG.durationMs - (elapsed ?? 0)) / 1000); timer.textContent = String(seconds); timer.classList.toggle("is-urgent", seconds <= 3 && !preview);
      fishNodes.forEach((image, slot) => { const fish = stateAt(view.movement_seed, slot, run.slots[slot], movementTime); image.style.left = `${fish.x * 100}%`; image.style.top = `${fish.y * 100}%`; image.style.zIndex = String(fish.z + 1); image.style.transform = `translate(-50%, -50%) scaleX(${fish.facingRight ? -1 : 1})`; });
      field.classList.toggle("is-finished", view.completed || elapsed !== null && elapsed >= CONFIG.durationMs); field.classList.toggle("is-preview", preview);
    ...
    function update(next) {
      ...
      if (key !== nextKey) { key = nextKey; events = [...(next.poisonFishView?.events ?? [])]; lastSentLength = events.length; finalSent = next.poisonFishView?.completed === true; retryAt = 0; finalizeRetryAt = 0; cachedEventCount = -1; }
